...
mkdir "%MODS%"
set FALLO=0
echo Descargando 96 mods (unos 310 MB, solo la primera vez)...
echo [1/96] AI Improvements: Performance Tuning
...
echo [11/96] Better Advancements
...
echo [13/96] Bountiful
curl -fsSL --retry 3 -o "%MODS%\Bountiful-6.0.4+1.20.1-forge.jar" "https://cdn.modrinth.com/data/BpwWFOVM/versions/EM7Su3PD/Bountiful-6.0.4%%2B1.20.1-forge.jar" || set FALLO=1
echo [14/96] Caelus API
...
echo [27/96] Dragon Mounts: Legacy
...
echo [28/96] Dungeons and Taverns
...
echo [35/96] FriendsFoes (Forge/NeoForge)
...
echo [43/96] Iron's Spells 'n Spellbooks
...
echo [45/96] Just Enough Items (JEI)
...
echo [46/96] Kambrik
curl -fsSL --retry 3 -o "%MODS%\Kambrik-6.1.1+1.20.1-forge.jar" "https://cdn.modrinth.com/data/zfbCkvdZ/versions/er56DwLJ/Kambrik-6.1.1%%2B1.20.1-forge.jar" || set FALLO=1
echo [47/96] Kotlin for Forge
...
echo [48/96] L_Ender's Cataclysm
...
echo [49/96] Lightman's Currency
curl -fsSL --retry 3 -o "%MODS%\lightmanscurrency-1.20.1-2.3.0.5.jar" "https://cdn.modrinth.com/data/wvMc8AJt/versions/eAALa47O/lightmanscurrency-1.20.1-2.3.0.5.jar" || set FALLO=1
echo [50/96] Lionfish-API
...
echo [56/96] Moog's Structure Lib (moogs_structures)
...
echo [57/96] MVS - Moog's Voyager Structures
...
echo [63/96] Open Parties and Claims
...
echo [65/96] Packet Fixer
curl -fsSL --retry 3 -o "%MODS%\packetfixer-3.3.2-1.18-1.20.4-merged.jar" "https://cdn.modrinth.com/data/c7m1mi73/versions/9F4NGhGR/packetfixer-3.3.2-1.18-1.20.4-merged.jar" || set FALLO=1
echo [66/96] Patchouli
...
echo [70/96] Repurposed Structures - Neoforge/Forge
...
echo [75/96] Sophisticated Backpacks
...
echo [83/96] Traveler's Backpack
...
echo [85/96] When Dungeons Arise
...
echo [89/96] YUNG's Better Desert Temples
...
echo [90/96] YUNG's Better Dungeons
...
echo [91/96] YUNG's Better Mineshafts
...
echo [92/96] YUNG's Better Nether Fortresses
...
echo [93/96] YUNG's Better Ocean Monuments
...
echo [94/96] YUNG's Better Strongholds
